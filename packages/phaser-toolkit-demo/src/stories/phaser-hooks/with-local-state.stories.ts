...
import { PreviewScene, PreviewScene2 } from './with-local-state/simple-example';
import { defaultLightTheme, HUDINI_KEY, HudiniPlugin, HudiniPluginData } from 'hudini';
import { PreviewScene2OnChange, PreviewSceneOnChange } from './with-local-state/on-change-example';

const ID = 'phaser-hooks-game-with-local-state';

const meta: Meta = {
    title: 'Phaser Hooks/With Local State/Basic Example',
    parameters: {
        docs: {
            description: {
                component: 'Basic example using Phaser Hooks for state management',
            },
        },
    },
};
