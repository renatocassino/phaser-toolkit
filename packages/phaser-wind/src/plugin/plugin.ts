 ...
 */
export class PhaserWindPlugin<T extends BaseThemeConfig> extends Plugins.BasePlugin {
    ...
    override init({ theme, darkMode = false }: PhaserWindPluginData): void {
        if (!theme) {
            // @ts-ignore
            this.theme = darkMode ? defaultDarkTheme : defaultLightTheme as T & BaseThemeConfig;
            return;
        }
    ...
    public get color(): null {
        this.getToken('colors.primary');
        return null;
    }
