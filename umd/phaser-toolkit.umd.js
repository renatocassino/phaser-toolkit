(function (global, factory) {
     ...
     */
    const createLogPrefix$1 = (operation) => {
        const timestamp = formatDateTime$1();
     ...
     */
    const createLogStyles$1 = (operation) => {
        const styles = [
     ...
     */
    const logStateInit$1 = (key, initialValue) => {
        const prefix = createLogPrefix$1('STATE_INIT');
        const styles = createLogStyles$1('STATE_INIT');
        console.groupCollapsed(`${prefix} Initializing state "${key}"`, ...styles);
        console.log('🔧 Key:', key);
        console.log('📦 Initial Value:', initialValue);
        console.log('⏰ Timestamp:', formatDateTime$1());
        console.groupEnd();
    };
    /**
     * Log state value retrieval
     */
    const logStateGet$1 = (key, value) => {
        const prefix = createLogPrefix$1('STATE_GET');
        const styles = createLogStyles$1('STATE_GET');
        console.log(`${prefix} Getting state "${key}":`, ...styles, value);
    };
    /**
     * Log state value update
     */
    const logStateSet$1 = (key, oldValue, newValue) => {
        const prefix = createLogPrefix$1('STATE_SET');
        const styles = createLogStyles$1('STATE_SET');
     ...
     */
    const logEventListenerAdd$1 = (key, event, callback) => {
        const prefix = createLogPrefix$1('EVENT_ADD');
        const styles = createLogStyles$1('EVENT_ADD');
     ...
     */
    const logEventListenerRemove$1 = (key, event, callback) => {
        const prefix = createLogPrefix$1('EVENT_REMOVE');
        const styles = createLogStyles$1('EVENT_REMOVE');
     ...
     */
    const logClearListeners$1 = (key) => {
        const prefix = createLogPrefix$1('CLEAR_LISTENERS');
        const styles = createLogStyles$1('CLEAR_LISTENERS');
     ...
     */
    const logError$1 = (operation, error, context) => {
        const prefix = createLogPrefix$1(operation);
        const styles = createLogStyles$1(operation);
        ...
        console.error('⏰ Timestamp:', formatDateTime$1());
        console.groupEnd();
    };
    /**
     * Log warning with context
     */
    const logWarning$1 = (operation, message, context) => {
        const prefix = createLogPrefix$1(operation);
        const styles = createLogStyles$1(operation);
        ...
        console.warn('⏰ Timestamp:', formatDateTime$1());
        console.groupEnd();
    };
     ...
     */
    const get$1 = (registry, key, debug) => {
        const value = registry.get(key);
        if (debug) {
            logStateGet$1(key, value);
        }
        return value;
    };
    /**
     ...
     */
    const set$1 = (registry, key, value, debug, validator) => {
        ...
        if (debug) {
            logStateSet$1(key, currentValue, newValue);
        }
    };
    /**
     ...
     */
    const patch$1 = (registry, key, value, debug, validator) => {
        const currentValue = registry.get(key);
     ...
     */
    const onChange$1 = (registry, key, debug, callback) => {
        logWarning$1('DEPRECATED_ONCHANGE', "onChange callback is deprecated in phaser-hooks. Use .on('change', callback) or .once('change', callback) instead.", { key });
        ...
        (_parent, key, value, previousValue) => {
            if (debug) {
                logStateSet$1(key, previousValue, value);
            }
            try {
                callback(value, previousValue);
            }
            catch (error) {
                logError$1('ONCHANGE_CALLBACK_ERROR', error, { key });
            }
        });
    };
    /**
     ...
     */
    const on$1 = (registry, event, key, debug, callback) => {
        if (event !== 'change') {
            throw new Error('[withStateDef] Invalid event. Only "change" is supported.');
        }
        if (debug) {
            logEventListenerAdd$1(key, event, callback);
        }
        registry.events.on(`changedata-${key}`, callback);
        return () => {
            if (debug) {
                logEventListenerRemove$1(key, event, callback);
            }
     ...
     */
    const validateHook$1 = (scene, options, key) => {
        if (!scene) {
     ...
     * @throws {Error} If the validator returns false or an error message
     */
    const initializeState$1 = (registry, key, debug, validator, initialValue) => {
        ...
        if (!registry.has(key) && initialValue !== undefined) {
            ...
            if (debug) {
                logStateInit$1(key, initialValue);
            }
        }
    };
    /**
     ...
     * @throws {Error} If event is not 'change'
     */
    const once$1 = (registry, event, key, debug, callback) => {
        if (event !== 'change') {
            throw new Error('[withStateDef] Invalid event. Only "change" is supported.');
        }
        if (debug) {
     ...
     * @throws {Error} If event is not 'change'
     */
    const off$1 = (registry, event, key, debug, callback) => {
        if (event !== 'change') {
            throw new Error('[withStateDef] Invalid event. Only "change" is supported.');
        }
        registry.events.off(`changedata-${key}`, callback);
     ...
     * @param {boolean} debug - Whether to log debug info
     */
    const clearListeners$1 = (registry, key, debug) => {
        registry.events.removeAllListeners(`changedata-${key}`);
        if (debug) {
            logClearListeners$1(key);
        }
    };
    /**
     ...
     */
    const withStateDef$1 = (scene, key, initialValue, options = {}) => {
        validateHook$1(scene, options, key);
        ...
        initializeState$1(registry, key, debug, validator, initialValue);
        return {
            /**
             * Gets the current state value.
             * @returns {T}
             */
            get: () => get$1(registry, key, debug),
            /**
             ...
             */
            set: (value) => set$1(registry, key, value, debug, validator),
            /**
             ...
             */
            patch: (value) => patch$1(registry, key, value, debug, validator),
            /**
             * Registers a callback to be called whenever the state changes (DEPRECATED).
             * @param {StateChangeCallback<T>} callback
             */
            onChange: (callback) => onChange$1(registry, key, debug, callback),
            /**
             ...
             */
            on: (event, fn) => on$1(registry, event, key, debug, fn),
            /**
             ...
             * @returns {() => void} Unsubscribe function
             */
            once: (event, fn) => once$1(registry, event, key, debug, fn),
            /**
             * Removes an event listener for the state.
             * Only the 'change' event is supported.
             * @param {'change'} event
             * @param {Function} fn
             */
            off: (event, fn) => off$1(registry, event, key, debug, fn),
            /**
             * Removes all event listeners for this state.
             */
            clearListeners: () => clearListeners$1(registry, key, debug),
        };
    };
    ...
    const withLocalState = (scene, key, initialValue, options) => {
        ...
        return withStateDef$1(scene, localKey, initialValue, {
            ...options,
            global: false,
        });
    };
     ...
     * @see {@link withStateDef} For low-level state management
     */
    const withGlobalState$1 = (scene, key, initialValue, options) => {
        if (!scene) {
     ...
     * ```
     */
    const withPersistentState$1 = (scene, key, initialValue, storageKey, storageType = 'local') => {
        ...
        // @ts-ignore
        const state = withGlobalState$1(scene, key, storedValue);
    ...
    var PhaserHooks = /*#__PURE__*/Object.freeze({
        ...
        withGlobalState: withGlobalState$1,
        withLocalState: withLocalState,
        withPersistentState: withPersistentState$1,
        withStateDef: withStateDef$1,
     ...
     */
    const convertRgbToNumber$1 = (rgbValue) => {
        const matches = rgbValue.match(RGB_REGEX$1);
        if (!matches) {
     ...
     */
    const convertColorValueToNumber$1 = (colorValue) => {
        if (colorValue.startsWith('#')) {
            return convertHexToNumber$1(colorValue);
        }
        return convertRgbToNumber$1(colorValue);
    };
    /**
     ...
     */
    const createColor$1 = (themeColors) => {
        /**
        ...
        const rgb = (color) => {
            ...
            if (typeof color === 'string' && isValidColor$1(color)) {
                return color;
            }
            ...
            if (parts.length === 2) {
                ...
                const shade = parts[1];
                const colorValue = palette$1[colorKey]?.[shade];
                if (!colorValue) {
                    if (isValidColor$1(color)) {
                        return color;
                    }
            ...
            const colorValue = palette$1[color];
            if (!colorValue) {
                if (isValidColor$1(color)) {
                    return color;
                }
        ...
        const hex = (color) => {
            // See note in rgb()
            if (typeof color === 'string' && isValidColor$1(color)) {
                return convertColorValueToNumber$1(color);
            }
            const colorFromTheme = getValueFromTheme(color);
            if (colorFromTheme) {
            ...
            if (parts.length === 2) {
                ...
                return convertColorValueToNumber$1(colorValue);
            }
            const colorToConvert = palette$1[color];
            if (isValidColor$1(colorToConvert)) {
                return convertColorValueToNumber$1(colorToConvert);
            }
            throw new Error(`Color token "${color}" not found`);
        };
        const api = {
     ...
     * f.css('xxl'); // '28px'
     */
    const createFontSize$1 = (themeFontSizes) => {
        const fontmap = {
            ...fontSizeMap$1,
            ...themeFontSizes,
        };
    ...
    const FontSize$1 = createFontSize$1(undefined);
    ...
    const createFont$1 = (fonts, fontSizes) => {
        const map = {
            ...fontMap$1,
            ...fonts,
        };
        const fontSizeApi = createFontSize$1(fontSizes);
        const getFamily = (key) => {
            if (key.includes('.')) {
        ...
        return {
            ...
            getAvailableFonts: () => {
                return Array.from(new Set([...Object.keys(fontMap$1), ...Object.keys(map)]));
            },
        };
    };
     ...
     * r.css('card'); // '12px'
     */
    const createRadius$1 = (themeRadius) => {
        const map = {
            ...radiusMap$1,
            ...themeRadius,
        };
    ...
    const Radius$1 = createRadius$1(undefined);

    /** Default shadow presets. */
    const defaultShadowMap$1 = {
    ...
    const Shadow$1 = createShadow$1(undefined);
     ...
     * s.px('gutter'); // 24
     */
    const createSpacing$1 = (themeSpacing) => {
        const map = {
            ...spacingMap$1,
            ...themeSpacing,
        };
    ...
    const Spacing$1 = createSpacing$1(undefined);
    ...
    const createTheme$1 = (theme) => merge({}, defaultLightTheme$1, theme);

    const PHASER_WIND_KEY$1 = 'pw';
    /**
     * Phaser Wind Plugin class that manages theme configuration
     * @extends Plugins.BasePlugin
     */
    let PhaserWindPlugin$1 = class PhaserWindPlugin extends Phaser$1.Plugins.BasePlugin {
        colorInstance = null;
        ...
        constructor(pluginManager) {
            super(pluginManager);
            this.theme = defaultLightTheme$1;
        }
        /**
        ...
        init({ theme, darkMode = false }) {
            ...
            this.colorInstance = createColor$1(this.theme.colors);
            this.fontSizeInstance = createFontSize$1(this.theme.fontSizes);
            this.spacingInstance = createSpacing$1(this.theme.spacing);
            this.radiusInstance = createRadius$1(this.theme.radius);
            this.fontInstance = createFont$1(this.theme.fonts, this.theme.fontSizes);
            this.shadowInstance = createShadow$1(this.theme.effects);
        }
        /**
    ...
    let SceneWithPhaserWind$1 = class SceneWithPhaserWind extends Phaser.Scene {
        /**
         *
    ...
    var PhaserWind = /*#__PURE__*/Object.freeze({
        ...
        Color: Color$1,
        FontSize: FontSize$1,
        PHASER_WIND_KEY: PHASER_WIND_KEY$1,
        PhaserWindPlugin: PhaserWindPlugin$1,
        Radius: Radius$1,
        SceneWithPhaserWind: SceneWithPhaserWind$1,
        Shadow: Shadow$1,
        Spacing: Spacing$1,
        createColor: createColor$1,
        createFont: createFont$1,
        createFontSize: createFontSize$1,
        createRadius: createRadius$1,
        createShadow: createShadow$1,
        createSpacing: createSpacing$1,
        createTheme: createTheme$1,
        defaultDarkTheme: defaultDarkTheme$1,
        defaultLightTheme: defaultLightTheme$1,
        defaultShadowMap: defaultShadowMap$1,
        fontMap: fontMap$1,
        fontSizeMap: fontSizeMap$1,
        isValidColor: isValidColor$1,
        palette: palette$1,
        radiusMap: radiusMap$1,
        spacingMap: spacingMap$1
    });
     ...
     */
    const convertToFontAwesome$1 = (codeIcon) => {
        return String.fromCharCode(parseInt(codeIcon, 16));
    };

    const getIconChar$1 = (key) => {
        if (!fontIcons$1[key]) {
            throw new Error(`Icon ${key} not found in fontIcons`);
        }
        return convertToFontAwesome$1(fontIcons$1[key].hexCode);
    };
    const getIconStyles$1 = (key) => {
        if (!fontIcons$1[key]) {
            throw new Error(`Icon ${key} not found in fontIcons`);
        }
        return new Set(fontIcons$1[key].acceptedStyles);
    };
    ...
    const getIconHex = (key) => {
        ...
        return fontIcons$1[key].hexCode;
    };
     ...
     */
    let IconText$1 = class IconText extends Phaser$1.GameObjects.Text {
        currentIconStyle = 'solid';
        icon;
        constructor({ scene, x, y, icon, size = 16, style = {}, iconStyle = 'solid', }) {
            super(scene, x, y, getIconChar$1(icon), {
                fontSize: `${size}px`,
                ...style,
            });
            this.icon = icon;
        ...
        setIcon(icon, opts) {
            this.setText(getIconChar$1(icon));
            this.icon = icon;
            if (opts?.iconStyle) {
        ...
        applyIconStyle(iconStyle) {
            ...
            const availableStyles = getIconStyles$1(this.icon);
    ...
    var FontAwesome = /*#__PURE__*/Object.freeze({
        __proto__: null,
        IconText: IconText$1,
        fontIcons: fontIcons$1,
        getIconChar: getIconChar$1,
        getIconHex: getIconHex,
        getIconStyles: getIconStyles$1,
        loadFont: loadFont
    });
    ...
    let VirtualJoystick$1 = class VirtualJoystick extends Phaser__namespace.GameObjects.Container {
         ...
         * ```
         */
        constructor({ scene, deadZone, baseArea, stick, stickIcon, bounds, enableWithoutTouch = false, }) {
            super(scene, 0, 0);
            this.deadZoneConfig = {
         ...
         */
        emitMove(x, y) {
            this.scene.events.emit(exports.VirtualJoystickEvents.UPDATE, { x, y });
        }
        /**
    ...
    exports.fontIcons = fontIcons$1;
    exports.getDefaultBounds = getDefaultBounds;
    exports.getHudini = getHudini;
    exports.getIconChar = getIconChar$1;
    exports.getIconHex = getIconHex;
    exports.getIconStyles = getIconStyles$1;
    ...
    exports.withGlobalState = withGlobalState$1;
    exports.withLocalState = withLocalState;
    exports.withPersistentState = withPersistentState$1;
    exports.withStateDef = withStateDef$1;
